...
solution_2 = 0
sums = deque()
current_sums = deque([[1, data[0]]])
for i in range(1, len(data)):
    for sum in current_sums:
        if sum[0] < 3:
            sum[0] += 1
            sum[1] += data[i]
    current_sums.append([1, data[i]])

    while current_sums[0][0] == 3:
        sums.append(current_sums.popleft())

start_sum = sums.popleft()
while start_sum:
    if len(sums) and sums[0][1] > start_sum[1]:
        solution_2 += 1
    if len(sums):
        start_sum = sums.popleft()
    else:
